...
test("open table uses one-viewport shell with overlay drawers", async () => {
  ...
  assert.match(css, /@media \(max-width: 680px\)[\s\S]*\.party-drawer,[\s\S]*\.state-drawer,[\s\S]*\.log-drawer,[\s\S]*\.character-drawer,[\s\S]*\.settings-drawer[\s\S]*transform: translateY\(100%\)/);
  ...
  assert.match(app, /playerSetupPanel\?\.classList\.toggle\("hidden", Boolean\(localPlayer\)\)/);
