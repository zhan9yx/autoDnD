...
- Status: open in `0012-continuous-depth-assets`
- Found by: v11 QA closeout and 0012 maturity review.
- Reproduction: Asset scale remains below the 3000+ asset and 500-scene targets. Earlier first-time setup/action hierarchy, purchase/use feedback, soundscape visibility, and tool-like item semantics subitems are now superseded by the current status below.
...
- Current 0012 status: the no-scroll UI and production-depth scene-selection blockers are fixed, the latest confirmed engineering gates are green, and the `#marketStatus` free-time copy CSS issue is fixed by focused code/static checks plus browser regression. Static/API serving is reverified and the latest desktop/mobile browser reports passed with `issues=[]`. Worker H closed the soundscape visibility/copy subset with focused bilingual/static tests (`node --check public/app.js`, `node --check public/i18n.js`, and `node --test tests/bilingualUi.test.js tests/playerUiAccess.test.js` passed 14/14). Worker H also closed the tool-like item semantics subset: non-slotted tools such as `暴风提灯` are usable from the backpack, explicitly non-equippable, and expose localized equip/use reasons (`node --check src/core/itemCatalog.js`, `node --check public/i18n.js`, and `node --test tests/itemCatalog.test.js tests/bilingualUi.test.js` passed 27/27). Worker H then closed the purchase/use/backpack feedback subset: buy/use/equip/sell/reward gain now give localized confirmation plus backpack/equipment-refresh cues through existing status/toast surfaces (`node --check public/app.js`, `node --check public/i18n.js`, and `node --test tests/itemCatalog.test.js tests/bilingualUi.test.js` passed 28/28). Worker H also closed the equipment-summary item-name subset: existing character/player summary surfaces show actual equipped item names where present (`node --check public/app.js` and `node --test tests/itemCatalog.test.js tests/staticUiStructure.test.js` passed 19/19); a follow-up review fixed the off-hand label so shields and focuses render under `Off hand`/`副手` instead of the narrower `Focus`/`法器`. Worker H closed the first-time setup/action hierarchy subset: Start scene is primary in the topbar, Join table is grouped with Guide, and Start scene exposes localized disabled/ready title and aria reasons (`node --check public/app.js`, `node --check public/i18n.js`, and `node --test tests/staticUiStructure.test.js tests/bilingualUi.test.js tests/noScrollUi.test.js tests/playerUiAccess.test.js` passed 24/24). Worker H closed the progression-loop runtime/static subset: XP gain levels up, unlocks progression benefits, scrolls learn spells, equip updates equipment summary/stat deltas, and My character/State summary paths are statically covered (`node --check src/core/gameEngine.js`, `node --check src/core/localization.js`, `node --check public/app.js`, `node --test tests/localization.test.js` passed 8/8, `node --test tests/gameEngine.test.js tests/itemCatalog.test.js tests/staticUiStructure.test.js` passed 34/34, `node --test tests/rules.test.js tests/guide.test.js` passed 16/16). Public-launch readiness and asset-scale expansion remain open.
...
- Status: fixed-local-evidence-attached in `0015-continuous-hardening`
...
- Current 0015 status: protected-room browser recheck evidence from 0013 is useful input, automated coverage is broad, and the 0015 refresh-recovery P1 has local fix evidence. The 0015 consolidated browser acceptance pack now exists in `docs/qa/0015-consolidated-browser-acceptance.md`, with 30 screenshots and a machine-readable report under `/private/tmp/aidm-0015-consolidated-browser-final3/`. It covers desktop/mobile, open/password/host-approval rooms, chat/action, state/log/replay, market/backpack, audio settings, refresh recovery, and visible secret-safety checks.
- Close condition: local evidence condition is met. Public-launch prerequisites remain separate and are tracked under `BUG-0013` and `docs/RELEASE_GATES.md`.
...
- Current gate: `tests/publicReadinessGates.test.js` requires the 0015 gate package and fails if the public-readiness gates are marked passed in this change. Local refresh-recovery and 0013 auth/access protected-room browser evidence reduce local browser risk, but do not close `GATE-002` or any public-readiness gate.
